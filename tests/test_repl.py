...
from disnake.ext.jishaku.repl import AsyncCodeExecutor, Scope, get_parent_var, get_var_dict_from_ctx
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "_cool_data", [45], None, marks=pytest.mark.xfail(raises=NameError, strict=True)
        ),
...
async def test_executor_advanced(
    code: str, expected: list[int | None], arg_dict: dict[str, int] | None, scope: Scope
):

...
from disnake.ext.jishaku.paginators import PaginatorInterface, WrappedFilePaginator, use_file_check
            ...
            with open(path, "rb") as file:
                if use_file_check(ctx, size):
                    if line_span:
                        ...
                        await ctx.send(
                            file=disnake.File(filename=pathlib.Path(file.name).name, fp=file)
                        )
                else:
    ...
    async def jsk_curl(self, ctx: ContextA, url: str):
        ...
        async with ReplResponseReactor(ctx.message):
            ...
            if use_file_check(ctx, len(data)):  # File "full content" preview limit
                ...
                await ctx.send(
                    file=disnake.File(filename=f"response.{language or 'txt'}", fp=io.BytesIO(data))
                )
            else:
                try:

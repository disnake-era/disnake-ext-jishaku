...
from disnake.ext.jishaku.math import mean_stddev
from disnake.ext.jishaku.modules import ExtensionConverter
from disnake.ext.jishaku.repl import inspections
...
class ManagementFeature(Feature):
        ...
        for extension in itertools.chain(*extensions):
            ...
            try:
                ...
                if isinstance(exc, commands.ExtensionFailed) and exc.__cause__:
                    cause = exc.__cause__
                    traceback_data = "".join(
                        traceback.format_exception(type(cause), cause, cause.__traceback__, 8)
                    )
                else:
                    traceback_data = "".join(
    ...
    async def jsk_shutdown(self, ctx: ContextA):
        ...
        ellipse_character = (
            "\N{BRAILLE PATTERN DOTS-356}" if Flags.USE_BRAILLE_J else "\N{HORIZONTAL ELLIPSIS}"
        )
    ...
    async def jsk_sync(self, ctx: ContextA, *targets: str):
        ...
        for guild in guilds:
            ...
            if translator:
                payload = [
                    await command.get_translated_payload(translator) for command in slash_commands
                ]
            else:
            ...
            try:
                ...
                if guild:
                    ...
                    paginator.add_line(
                        f"\N{SATELLITE ANTENNA} Synced {len(synced)} global commands", empty=True
                    )

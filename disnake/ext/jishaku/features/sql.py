    ...
    class AsyncpgConnectionAdapter(Adapter[typing.Union[asyncpg.Connection, asyncpg.pool.Pool]]):
        ...
        async def table_summary(
            ...
            for record in await self.connection.fetch(  # type: ignore
                ...
                table_name: str = (
                    f"{record['table_catalog']}.{record['table_schema']}.{record['table_name']}"  # type: ignore
                )
...
class SQLFeature(Feature):
    ...
    async def jsk_sql_fetchrow(self, ctx: ContextA, *, query: str):
        ...
        text = tabulate(
            {key: [value] for key, value in output.items()}, headers="keys", tablefmt="psql"
        )

        if use_file_check(ctx, len(text)):
            await ctx.reply(
                file=disnake.File(filename="response.txt", fp=io.BytesIO(text.encode("utf-8")))
            )
        else:

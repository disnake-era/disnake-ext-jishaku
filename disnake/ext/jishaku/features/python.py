...
from disnake.ext.jishaku.codeblocks import Codeblock, codeblock_converter
from disnake.ext.jishaku.exception_handling import ReplResponseReactor
from disnake.ext.jishaku.features.baseclass import Feature
from disnake.ext.jishaku.flags import Flags
from disnake.ext.jishaku.formatting import MultilineFormatter
from disnake.ext.jishaku.functools import AsyncSender
from disnake.ext.jishaku.math import format_bargraph, format_stddev
from disnake.ext.jishaku.paginators import PaginatorInterface, WrappedPaginator, use_file_check
from disnake.ext.jishaku.repl import (
    AsyncCodeExecutor,
    Scope,
    all_inspections,
    create_tree,
...
from disnake.ext.jishaku.types import ContextA
...
class PythonFeature(Feature):
    ...
    async def jsk_python_result_handling(self, ctx: ContextA, result: typing.Any):  # pylint: disable=too-many-return-statements
        ...
        if len(result) <= 2000:
            ...
            return await ctx.send(result, allowed_mentions=disnake.AllowedMentions.none())
        ...
        if use_file_check(ctx, len(result)):  # File "full content" preview limit
            ...
            return await ctx.send(
                file=disnake.File(filename="output.py", fp=io.BytesIO(result.encode("utf-8")))
            )
            ...
            async with ReplResponseReactor(ctx.message):
                with self.submit(ctx):
                    executor = AsyncCodeExecutor(
                        argument.content, scope, arg_dict=arg_dict, convertables=convertables
                    )
    ...
    @Feature.Command(
        parent="jsk", name="py_inspect", aliases=["pyi", "python_inspect", "pythoninspect"]
    )
                ...
                async with ReplResponseReactor(ctx.message):
                    with self.submit(ctx):
                        ...
                        for lineno in sorted(line_timings.keys()):
                            ...
                            lines.append(
                                "\u001b[0m" + color + line if Flags.use_ansi(ctx) else line
                            )
                        ...
                        await ctx.send(
                            ...
                            file=disnake.File(
                                filename="lines.ansi", fp=io.BytesIO("".join(lines).encode("utf-8"))
                            ),
                        )
    ...
    async def jsk_disassemble(self, ctx: ContextA, *, argument: codeblock_converter):  # type: ignore
        ...
        async with ReplResponseReactor(ctx.message):
            ...
            if use_file_check(ctx, len(text)):  # File "full content" preview limit
                await ctx.send(
                    file=disnake.File(filename="dis.py", fp=io.BytesIO(text.encode("utf-8")))
                )
            else:
    ...
    async def jsk_ast(self, ctx: ContextA, *, argument: codeblock_converter):  # type: ignore
        ...
        async with ReplResponseReactor(ctx.message):
            ...
            await ctx.send(
                file=disnake.File(filename="ast.ansi", fp=io.BytesIO(text.encode("utf-8")))
            )
            ...
            try:
                async with ReplResponseReactor(ctx.message):
                    with self.submit(ctx):
                        ...
                        await ctx.send(
                            file=disnake.File(
                                filename="specialist.ansi", fp=io.BytesIO(text.encode("utf-8"))
                            )
                        )
